...
if(isset($_POST['delete'])){
   ...
   $delete_cart_item = $conn->prepare("DELETE FROM `panier` WHERE id = ?");
   $delete_cart_item->execute([$cart_id]);
}

if(isset($_GET['delete_all'])){
   $delete_cart_item = $conn->prepare("DELETE FROM `panier` WHERE user_id = ?");
...
if(isset($_POST['update_qty'])){
   ...
   $update_qty = $conn->prepare("UPDATE `panier` SET quantity = ? WHERE id = ?");
...
<section class="products shopping-cart">
   ...
   <h3 class="heading">Shopping panier</h3>
   ...
   $grand_total += $sub_total;
      ...
      echo '<p class="empty">your panier is empty</p>';
   }
   ?>
   </div>
